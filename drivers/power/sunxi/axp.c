...
int  axp_set_charge_onoff(int onoff)
{
	return sunxi_axp_dev[CHARGER_PMU]->set_charge_onoff(onoff);
}
#endif
...
#ifdef CONFIG_SUN8IW12P1_NOR
	int bat_ocv;
	/*close battery charged*/
	sunxi_axp_dev[0]->set_charge_onoff(0);
	bat_ocv = sunxi_axp_dev[0]->probe_battery_ocv_vol();
#endif
...
#ifdef CONFIG_SUN8IW12P1_NOR
	/*if coulombmeter not ready or ocv vol was low, re-enable coulombmenter*/
	if(ratio < 1 || bat_ocv <= safe_vol || bat_vol <safe_vol)
#else
	if(ratio < 1)
#endif
	{
...
int axp_set_supply_vol_byregulator(const char *id, int onoff, int vol)
{
	char main_key[32];
	char pmu_type[32];
	char vol_type[32];
	...
	/* para vol = 0  indcate not set vol,only open or close  voltage switch */
	ret = axp_set_supply_status_byname(pmu_type, vol_type, vol, onoff);
	if (ret != 0)
		printf("error: supply regelator %s=id, pmu_type=%s_%s onoff=%d fail\n", id, pmu_type, vol_type, onoff);
